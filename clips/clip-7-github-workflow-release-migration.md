...
Updated: 2026-02-08
Status: Implemented
Commits: ci(release): migrate llm-council GitHub maintenance workflows and prepare v0.1.1; fix(ci): build artifacts before electron smoke tests; test(electron): stabilize smoke checks under headless CI; test(electron): wait for council bridge readiness in IPC helpers
---
...
6. IPC helper 时序保障：
   - 对依赖 `window.council` 的测试路径先等待 bridge ready，再执行 IPC 断言
